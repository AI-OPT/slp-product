...
public final class SearchConstants {
    ...
    public static class QUERY_SQL {
        ...
        public static final String FETCH_ATTR_INFO_SQL ="SELECT standed_prod_attr.ATTR_ID as attrID, standed_prod_attr.ATTR_VALUE_NAME as attrValue,standed_prod_attr.ATTRVALUE_DEF_ID as attrValueDefId,PROD_ATTR_DEF.ATTR_NAME as attrName FROM (product inner join standed_prod_attr on product.STANDED_PROD_ID = standed_prod_attr.STANDED_PROD_ID) inner join PROD_ATTR_DEF  on standed_prod_attr.ATTR_ID=PROD_ATTR_DEF.ATTR_ID where product.PROD_ID = ? AND standed_prod_attr.STATE='1' AND PROD_ATTR_DEF.STATE='1' ";
        public static final String FETCH_CATEGORY_INFO_SQL = "select PRODUCT_CAT_ID,PRODUCT_CAT_NAME,PARENT_PRODUCT_CAT_ID from product_cat a where a.PRODUCT_CAT_ID = ? AND STATE = '1'";
        public static final String FETCH_SALE_NUMBER_SQL = "select  SALE_NUM  from PROD_SALE_ALL where PROD_ID = ?";
        public static final String FETCH_IMAGE_FROM_SKU_SQL = "SELECT  PIC_TYPE, VFS_ID FROM PROD_CAT_ATTR catAttr, PROD_SKU_ATTR skuAttr, PROD_PICTURE picture WHERE catAttr.ATTR_ID = skuAttr.ATTR_ID AND skuAttr.ATTRVALUE_DEF_ID = picture.ATTRVALUE_DEF_ID AND catAttr.PRODUCT_CAT_ID = ? AND catAttr.ATTR_TYPE = '2' AND catAttr.STATE = '1' AND catAttr.IS_PICTURE = 'Y' AND skuAttr.SKU_ID = ? AND skuAttr.STATE='1' AND picture.IS_MAIN_PIC = 'Y' AND picture.STATE = '1' ";
        public static final String FETCH_IMAGE_FROM_PROD_SQL = "SELECT PIC_TYPE, VFS_ID,IS_MAIN_PIC FROM PROD_PICTURE WHERE  PROD_ID = ? AND STATE = '1' AND IS_MAIN_PIC='Y' ";
        ...
        public static final String PIC="SELECT  PIC_TYPE, VFS_ID FROM PROD_CAT_ATTR catAttr, PROD_SKU_ATTR skuAttr, PROD_PICTURE picture WHERE catAttr.ATTR_ID = skuAttr.ATTR_ID AND skuAttr.ATTRVALUE_DEF_ID = picture.ATTRVALUE_DEF_ID AND  catAttr.PRODUCT_CAT_ID = ? AND catAttr.ATTR_TYPE = '2' AND catAttr.STATE = '1' AND catAttr.IS_PICTURE = 'Y' AND skuAttr.SKU_ID = ? AND skuAttr.STATE='1' AND picture.IS_MAIN_PIC = 'Y' AND picture.STATE = '1' LIMIT 1";
        public static final String PRO="SELECT PIC_TYPE, VFS_ID FROM PROD_PICTURE WHERE PRO_PICTURE_ID IN(SELECT MIN(PRO_PICTURE_ID) FROM PROD_PICTURE WHERE  PROD_ID=? AND STATE = '1' AND IS_MAIN_PIC='Y' GROUP BY ATTRVALUE_DEF_ID)  ";
    }
}
