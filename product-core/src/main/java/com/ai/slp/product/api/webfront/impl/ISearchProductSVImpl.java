...
public class ISearchProductSVImpl implements ISearchProductSV {
	...
	// 获取搜索条数
	public int searchProductCount(ProductQueryRequest request) throws BusinessException, SystemException {
		IProductSearch productSearch = new ProductSearchImpl();
		String userid = "";
		if (!StringUtil.isBlank(request.getUserId())) {
			userid = request.getUserId();
	...
	// 获取条数
	@Deprecated
	public int queryProductPageCount(ProductQueryRequest request) throws BusinessException, SystemException {
		IProductSearch productSearch = new ProductSearchImpl();
		String userid = "";
		if (!StringUtil.isBlank(request.getUserId())) {
			userid = request.getUserId();
		...
		if (!StringUtil.isBlank(request.getAttrDefId()) && !StringUtil.isBlank(request.getBasicOrgIdIs())) {
			...
			if (!StringUtil.isBlank(request.getSaleNumOrderFlag())) {
				...
				if (!StringUtil.isBlank(request.getDistributionArea())) {
					productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
							request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
									.rechargeTypeNotIs(ProductHomeConstants.FILL_TYPE).tenantID(request.getTenantId())
									.attrValueDefID(request.getDistributionArea())
									.addOrderBy(ProductHomeConstants.ORDER_SALE_NUM_NAME, SortOrder.ASC)
									.basicOrgIdIs(request.getBasicOrgIdIs()).categoryIdIs(request.getProductCatId())
									.attrValueDefID(request.getAttrDefId()).build();
				} else {
					productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
							request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
									.rechargeTypeNotIs(ProductHomeConstants.FILL_TYPE).tenantID(request.getTenantId())
				...
				if (request.getPriceOrderFlag().equals(ProductHomeConstants.ORDER_ASC_ID)) {
					...
					if (!StringUtil.isBlank(request.getDistributionArea())) {
						productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
								request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
										.rechargeTypeNotIs(ProductHomeConstants.FILL_TYPE)
										.tenantID(request.getTenantId()).attrValueDefID(request.getDistributionArea())
										.addOrderBy(ProductHomeConstants.ORDER_PRICE_NAME, SortOrder.ASC)
										.basicOrgIdIs(request.getBasicOrgIdIs()).categoryIdIs(request.getProductCatId())
										.attrValueDefID(request.getAttrDefId()).build();
					} else {
				...
				if (!StringUtil.isBlank(request.getDistributionArea())) {
					productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
							request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
									.rechargeTypeNotIs(ProductHomeConstants.FILL_TYPE).tenantID(request.getTenantId())
									.attrValueDefID(request.getDistributionArea())
									.attrValueDefID(request.getAttrDefId()).categoryIdIs(request.getProductCatId())
									.addOrderBy(ProductHomeConstants.ORDER_SALE_NUM_NAME, SortOrder.ASC).build();
				} else {
					productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
							request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
									.rechargeTypeNotIs(ProductHomeConstants.FILL_TYPE).tenantID(request.getTenantId())
					...
					if (!StringUtil.isBlank(request.getDistributionArea())) {
						productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
								request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
										...
										.tenantID(request.getTenantId()).attrValueDefID(request.getDistributionArea())
										.attrValueDefID(request.getAttrDefId()).categoryIdIs(request.getProductCatId())
										.addOrderBy(ProductHomeConstants.ORDER_PRICE_NAME, SortOrder.ASC).build();
					} else {
						productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
								request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
										.rechargeTypeNotIs(ProductHomeConstants.FILL_TYPE)
										.tenantID(request.getTenantId()).attrValueDefID(request.getAttrDefId())
										.categoryIdIs(request.getProductCatId())
										.addOrderBy(ProductHomeConstants.ORDER_PRICE_NAME, SortOrder.ASC).build();
					}
				} else {
					// 如果配送地区不为空
					if (!StringUtil.isBlank(request.getDistributionArea())) {
						productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
								request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
										...
										.tenantID(request.getTenantId()).attrValueDefID(request.getDistributionArea())
										.attrValueDefID(request.getAttrDefId()).categoryIdIs(request.getProductCatId())
										.addOrderBy(ProductHomeConstants.ORDER_PRICE_NAME, SortOrder.DESC).build();
					} else {
						productSearchCriteria = new ProductSearchCriteria.ProductSearchCriteriaBuilder(
								request.getAreaCode(), ProductHomeConstants.TYPENATION_WIDE, user)
										.rechargeTypeNotIs(ProductHomeConstants.FILL_TYPE)
